---
title: 文档
description: React Docs UI 模板项目的文档入口页，汇总快速开始和测试页面
author: React Docs UI Team
createdAt: 2026-03-25
lastUpdated: 2026-03-27
---

# 文档

欢迎使用 React Docs UI 文档系统。

## 快速开始

- [介绍](/docs/guide/introduction) - 了解 React Docs UI 的核心理念
- [安装](/docs/guide/installation) - 学习如何安装和配置
- [快速上手](/docs/guide/quick-start) - 在 5 分钟内创建你的第一个文档网站
- [配置说明](/docs/guide/configuration) - 深入了解配置选项
- [PDF Server 部署](/docs/guide/pdf-server) - 部署 PDF 生成服务

## 后端集成

- [接入概览](/docs/backend/overview) - 了解前端依赖哪些后端能力和配置开关
- [部署 Rust 后端](/docs/backend/deploy-rust-backend) - 使用官方参考实现启动后端服务
- [邮箱 SMTP 配置](/docs/backend/email-smtp) - 配置邮箱验证码发送能力
- [Google / GitHub OAuth](/docs/backend/oauth-google-github) - 配置第三方登录所需材料和回调地址

## 文档测试

- [MD 测试](/docs/test/test) - 测试 Markdown 渲染功能
- [KaTeX 测试](/docs/test/katex-test) - 测试数学公式渲染
- [MDX 演示](/docs/test/mdx-test) - 查看 MDX 组件使用示例
