...
模板默认会把 `/api` 代理到 `http://localhost:3000`，因此前端开发环境通常不需要额外改请求地址。

如果你暂时还没有后端，也可以先在 `public/config/site.yaml` 中关闭：
...
更细粒度的控制方式可见 [后端集成](/docs/backend/overview) 和 [配置说明](/docs/guide/configuration)。
