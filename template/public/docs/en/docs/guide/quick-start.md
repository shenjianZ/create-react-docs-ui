...
If you want auth, comments, bookmarks, analytics, or feedback, start the backend service as well.
...
The template proxies `/api` to `http://localhost:3000` by default, so local frontend requests do not need a separate host change.

If you do not have a backend yet, you can temporarily disable the whole backend UI in `public/config/site.en.yaml`:
...
See [Backend Integration](/docs/backend/overview) and [Configuration](/docs/guide/configuration) for the full setup.
